...
class AdaMerging(torch.nn.Module):
    ...
    def lambdas(self):
        ...
        task_lambdas = torch.zeros_like(task_lambdas)
...
dataset_samples = [2000, 2000, 2000, 2000, 2000, 2000, 2000]  # number of training samples for each dataset
...
if __name__ == "__main__":
    ...
    paramslist += [tuple(v.detach().clone().requires_grad_().to(args.device) for _, v in tv.task_vector_param_dict.items())  for i, tv in enumerate(task_vectors)] # task vectors
    ...
    # training lambdas
    optimizer = torch.optim.Adam(adamerging_mtl_model.module.collect_trainable_params(), lr=args.metalr, betas=(0.9, 0.999), weight_decay=0.)
    ...
    for idx, dataset_name in enumerate(args.dataset_names):
        ...
        indices = random.sample(range(len(ds_train)), 2000)  # 随机索引
    ...
    for meta_step in range(num_steps_meta):
        ...
        for step in range(num_steps_inner):
            for idx, it in enumerate(iters_train):
                ...
                #     print(grads[-1])
                # sys.exit(0)
                fast_weights[idx] = tuple(p - args.adalr * g for p, g in zip(fast_weights[idx], grads))
    ...
    folder = f"./cache/{readable}"
    os.makedirs(folder, exist_ok=True)
    torch.save(adamerging_mtl_model.module.lambdas_raw, f"./cache/{readable}/lambdas_raw.pt")
    torch.save(adamerging_mtl_model.module.paramslist_adapters, f"./cache/{readable}/adapters.pt")
    
    
    
    # train adapter and evaluate
    loss_fn = torch.nn.MSELoss()
    # loss_fn = torch.nn.L1Loss()
    acc_list = []
    num_steps_train_adapters = args.final_epochs
    for idx, (dataset_name, dataset_sample) in enumerate(zip(args.dataset_names, args.dataset_samples)):
        ...
        # adamerging_mtl_model.module.model.load_state_dict({name: param for name, param in zip(adamerging_mtl_model.module.names_adapters, adamerging_mtl_model.module.paramslist_adapters[idx])}, strict=False)
        # optimizer = torch.optim.Adam([param for name, param in adamerging_mtl_model.module.model.named_parameters() if "adapter" in name], lr=1e-2, betas=(0.9, 0.999), weight_decay=0.)
        optimizer = torch.optim.Adam(adamerging_mtl_model.module.paramslist_adapters[idx], lr=1e-3, betas=(0.9, 0.999), weight_decay=0.)
        ...
        for step in range(num_steps_train_adapters):
            ...
            # if local_rank == 0:   
            #     print(adamerging_mtl_model.module.paramslist_adapters[0][0])
            #     print(paramslist_adapters[1][1])
            for batch_idx, batch in enumerate(
                    pbar := tqdm(
                        loader_train, desc="AdapterTraining", leave=False, dynamic_ncols=True
                    )
            ):
                input_ids = batch["input_ids"].to(args.device)
                attention_mask = batch["attention_mask"].to(args.device)
                ...
                with torch.no_grad():
                    outputs_ft = models_ddp[idx](input_ids, attention_mask=attention_mask, output_hidden_states=True)
                loss = loss_fn(outputs.hidden_states[-1], outputs_ft.hidden_states[-1].detach())
                # grad = torch.autograd.grad(loss, adamerging_mtl_model.module.paramslist_adapters[idx])
                # print(grad)
                # sys.exit(0)


                # batch_size, seq_len, hidden_size = outputs.hidden_states[-1].size()
                # indices = last_token_indices.view(-1, 1, 1).expand(-1, 1, hidden_size)
                # last_hidden = outputs.hidden_states[-1].gather(1, indices).squeeze(1) 
                # last_hidden_ft = outputs_ft.hidden_states[-1].gather(1, indices).squeeze(1)
                # last_hidden_ft1 = outputs_ft.hidden_states[-1].gather(1, (indices+1)).squeeze(1)
                
                # loss_cls = loss_fn(last_hidden, last_hidden_ft.detach())
                ...
                optimizer.step()
                losses += loss.item() / 20
                # losses_cls += loss_cls.item() / 20
                if (batch_idx+1) % 20 == 0 and local_rank == 0:
                    logger.info(f"Adapter training, loss: {losses}")
                    # logger.info(f"Adapter training, cls loss: {losses_cls}")
                    losses = 0.0
                    losses_cls = 0.0
                # if local_rank == 0:
                    # print(adamerging_mtl_model.module.model.score.weight)
                    # print(adamerging_mtl_model.module.models[idx].module.score.weight)
                    # print(models_ddp[idx].module.score.weight)
                    # print(outputs.logits)
                    # print(outputs_ft.logits)
                    # print(outputs.hidden_states[-1][0, :3, :3])
                    # print(outputs_ft.hidden_states[-1][0, :3, :3])
                    # # print(loss_cls.item())
                    # print(last_hidden)
                    # print(last_hidden_ft)
                    # print(last_hidden_ft1)

                    # print(adamerging_mtl_model.module.paramslist_adapters[0][0])
                    # print(adamerging_mtl_model.module.paramslist_adapters[0][-1])    
                    # print(adamerging_mtl_model.module.paramslist_adapters[0][-1].grad)
                    # print(paramslist_adapters[1][1])
                    # print(adamerging_mtl_model.module.paramslist_adapters[1][1])
        ...
        with torch.no_grad():
            ...
            for batch in (
                    pbar := tqdm(
                        loader_val, desc="Evaluating", leave=False, dynamic_ncols=True
                    )
            ):
                input_ids = batch["input_ids"].to(args.device)
                attention_mask = batch["attention_mask"].to(args.device)
                ...
                # last_hidden_ft = outputs_ft.hidden_states[-1].gather(1, indices).squeeze(1)
                # last_hidden_ft = outputs_ft.hidden_states[-1][:, -1, :]
                # last_hidden_ft1 = outputs_ft.hidden_states[-1].gather(1, (indices+1)).squeeze(1)
                # last_hidden_ft1 = outputs_ft.hidden_states[-1][:, -1, :]
                # if local_rank == 0:
                #     print(adamerging_mtl_model.module.model.score.weight)
                #     print(models_ddp[idx].module.score.weight)
                #     print(outputs.logits)
                ...
                #     print(outputs.hidden_states[-1][0, :3, :3])
                #     print(outputs_ft.hidden_states[-1][0, :3, :3])
                #     print(loss_fn(outputs.hidden_states[-1], outputs_ft.hidden_states[-1]).item())
                #     # print(loss_cls.item())
                #     print(last_hidden)
                #     print(last_hidden_ft)
                #     print(last_hidden_ft1)
    ...
    # evaluate
    torch.save(adamerging_mtl_model.module.paramslist_adapters, f"./cache/{readable}/adapters_trained.pt")
    # evaluate(args, adamerging_mtl_model, tokenizer, logger)
    finish_time = time.time()
    if local_rank == 0:
        logger.info(f"Final acc list: {acc_list}, average acc: {sum(acc_list)/len(acc_list)}")
        logger.info(f"Total time cost: {finish_time - run_start_time} seconds.")
        logger.info(f"********** Run ends. **********\n\n")
